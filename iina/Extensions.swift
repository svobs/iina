...
extension NSScreen {
  ...
  static func log(_ label: String, _ screen: NSScreen?) {
    ...
    if #available(macOS 10.15, *) {
      ...
      Logger.log("\(label): \(screen.localizedName) visible frame \(screen.visibleFrame) supportsHDR=\(canEnableEDR) (\(maxPossibleEDR)) highestEDROnScreen=\(screen.maximumExtendedDynamicRangeColorComponentValue)")
    } else {
