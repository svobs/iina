...
class PlayerCore: NSObject {
  ...
  func stop() {
    ...
    mpv.queue.async { [self] in
      ...
      guard status.isNotYet(.stopped) else { return }
  ...
  func idleActiveChanged() {
    ...
    log.verbose("Got mpv 'idle-active' (isFileLoaded=\(isFileLoaded.yn), playerState=\(status))")
    ...
    } else if isFileLoaded || status.isAtLeast(.stopping) {
