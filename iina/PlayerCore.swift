...
//    case fileLoop
  ...
  func generateThumbnails() {
    ...
    guard let thumbWidth = determineWidthOfThumbnail() else { return }
  ...
  private func determineWidthOfThumbnail() -> Int? {
    ...
    if videoHeight > videoWidth {
      ...
      if requestedLength > videoHeight {
        ...
        thumbWidth = Int(Float(requestedLength) * (Float(videoHeight) / Float(videoWidth)))
        Logger.log("Video's height is longer than its width; adjusting thumbnail width to \(thumbWidth)", subsystem: subsystem)
      }
    } else {
