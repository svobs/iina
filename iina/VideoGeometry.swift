...
struct VideoGeometry: Equatable, CustomStringConvertible {
  ...
  var description: String {
    return "VidGeo(crop:\(selectedCropLabel.description.quoted)|\(cropRect?.description ?? "nil") aspect:\(codecAspectLabel.quoted)|override:\(userAspectLabel.quoted) rot:\(userRotation)°|total:\(totalRotation)° sizes:{raw:\(videoSizeRaw) CA:\(videoSizeCA) CAR:\(videoSizeCAR)|\(videoAspectCAR)})"
  }
